...
using ProductReviewAnalyzer.ReviewsScraper.Allo.Application.Commands.FetchReviews;

namespace ProductReviewAnalyzer.ReviewsScraper.Allo.API.Controllers;

[ApiController]
[Route("api/scraper/allo")]
    ...
    ILogger<ScraperController> logger) : ControllerBase
{
    ...
    public IActionResult FetchAll([FromBody] RequestDto dto)
    {
        ...
        _ = Task.Run(async () =>
        {
            ...
            {
                logger.LogDebug(
                    "Starting background task for Allo FetchReviews: RequestId={RequestId}",
                    dto.RequestId);

                var affectedCount = await mediator.Send(new FetchReviewsCommand(dto.RequestId, dto.Url));

                logger.LogInformation(
                    "Allo FetchReviews completed successfully: RequestId={RequestId}, ReviewsProcessed={Count}",
                    dto.RequestId,
                    affectedCount);
            }
            catch (Exception ex)
            {
                logger.LogError(
                    ex,
                    "Error while fetching reviews in Allo Scraper: RequestId={RequestId}, Exception={Message}",
                    dto.RequestId,
                    ex.Message);
            }
            finally
            {
                logger.LogDebug(
                    "Disposing scope for Allo Scraper: RequestId={RequestId}",
                    dto.RequestId);
